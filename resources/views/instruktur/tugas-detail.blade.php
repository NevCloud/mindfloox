                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-5">
                    ...
                    <section>
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <a href="{{ route('instruktur.tugas') }}" class="flex items-center gap-2 px-3 py-1.5 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-[#1A1A2E] transition-colors">
                        ...
                        <div class="card translate-none rounded-lg p-6 space-y-4">
                            ...
                            <div class="pb-4 border-b border-gray-200 dark:border-gray-700">
                                ...
                                @if($jawabanTugas->url_file)
                                    <a href="{{ asset('storage/' . $jawabanTugas->url_file) }}" target="_blank"
                                        ...
                                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-primary transition">
                            ...
                            @if($jawabanTugas->tugas->deskripsi)
                                <!-- Description Section -->
                                <div class="pb-4 border-b border-gray-200 dark:border-gray-700">
                                    <p class="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-2 uppercase tracking-wide">Deskripsi Tugas</p>
                                    <p class="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{{ $jawabanTugas->tugas->deskripsi }}</p>
                                </div>
                            @endif

                            <!-- Nilai Section -->
                            <div class="pt-2">
                                <p class="text-sm font-semibold text-gray-600 dark:text-gray-400 mb-3 uppercase tracking-wide">Masukkan Nilai</p>

                                @if($nilaiTugas)
                                    <div class="mb-4 p-3 rounded-xl bg-primary/5 dark:bg-primary/10 flex items-center justify-between">
                                        <p class="text-xs text-gray-500 dark:text-gray-400">Nilai saat ini (dinilai {{ $nilaiTugas->dinilai_pada?->format('d M Y') }})</p>
                                        <p class="text-2xl font-bold text-primary">{{ number_format($nilaiTugas->nilai_mentah, 1) }}</p>
                                    </div>
                                @endif

                                <form method="POST" action="{{ route('instruktur.evaluasi.tugas.nilai', $jawabanTugas->id) }}" class="flex items-end gap-3">
                                    @csrf
                                    <div class="flex-1">
                                        <label class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-1 block">
                                            Nilai (0–100)
                                        </label>
                                        <input type="number" name="nilai" min="0" max="100" step="0.1"
                                            value="{{ old('nilai', $nilaiTugas?->nilai_mentah) }}"
