...
<body x-data="uploadApp()" x-init="init()" class="relative bg-gray-50 dark:bg-[#0F0F1A]">
                    ...
                    <section>
                        <div class="card-fix p-6 space-y-6">
                            ...
                            @if ($item)
                                {{-- EDIT MODE --}}
                                @php
                                    $updateRoute = route('instruktur.kursus.konten.update', [
                                        $kursus->id,
                                        $tipe,
                                        $item->id,
                                    ]);
                                ...
                                <form method="POST" action="{{ $updateRoute }}" enctype="multipart/form-data"
                                    ...
                                    <input type="hidden" name="questions_json"
                                        x-bind:value="tipeMateri === 'kuis' ? JSON.stringify(questions) : ''">
                                    @include('instruktur.partials.upload-form-fields')
                                    @include('instruktur.partials.upload-form-actions')
                                </form>
                            @else
                                {{-- CREATE MODE --}}
                                <form method="POST" action="{{ route('instruktur.kursus.konten.store', $kursus->id) }}"
    ...
    @php
        ...
        $jsInitKuis = null;
        if ($tipe === 'kuis' && $item) {
            $jsInitKuis = $item->pertanyaanKuis
                ->map(function ($p) {
                    ...
                    return [
                        ...
                        'options' => $pilihanList->count()
                            ...
                            : ['A' => '', 'B' => '', 'C' => '', 'D' => ''],
                        'answer' =>
                            $p->tipe_pertanyaan === 'esai'
                                ? optional($p->kunciJawabanEsai->first())->teks_kunci
                                : $answerLetter,
                        'open' => false,
                        'case_sensitive' =>
                            $p->tipe_pertanyaan === 'esai'
                                ? (bool) optional($p->kunciJawabanEsai->first())->case_sensitive
                                : false,
                        'imageName' => '',
                    ];
                })
    ...
    <script>
        ...
        function uploadApp() {
            return {
                ...
                validationError: '',
                tanggalMulaiTugas: @json($tipe === 'tugas' && $item && $item->tanggal_mulai ? $item->tanggal_mulai->format('Y-m-d\TH:i') : ''),
                deadline: @json($tipe === 'tugas' && $item && $item->batas_waktu ? $item->batas_waktu->format('Y-m-d\TH:i') : ''),
                nilai: @json($item?->nilai ?? ''),
                batas_waktu_menit: @json($tipe === 'kuis' && $item ? $item->batas_waktu_menit : ''),
                tanggalMulaiKuis: @json($tipe === 'kuis' && $item && $item->tanggal_mulai ? $item->tanggal_mulai->format('Y-m-d\TH:i') : ''),
                batasWaktuKuis: @json($tipe === 'kuis' && $item && $item->batas_waktu ? $item->batas_waktu->format('Y-m-d\TH:i') : ''),
                fileName: '',
                ...
                submitForm(e) {
                    ...
                    if (this.tipeMateri === 'kuis') {
                        ...
                        if (unanswered.length > 0) {
                            ...
                            const nums = unanswered.map(({
                                i
                            }) => i + 1).join(', ');
                            this.validationError = `Pilih jawaban benar untuk soal nomor: ${nums}.`;
                            e.target.closest('section').previousElementSibling?.scrollIntoView({
                                behavior: 'smooth'
                            });
                            window.scrollTo({
                                top: 0,
                        ...
                        e.target.querySelector('[name="questions_json"]').value = JSON.stringify(
                            this.questions.map(q => ({
                                tipe_pertanyaan: q.tipe_pertanyaan || (q.type === 'essay' ? 'esai' :
                                    'pilihan_ganda'),
                                question: q.question,
                                options: q.options,
                                answer: q.answer,
