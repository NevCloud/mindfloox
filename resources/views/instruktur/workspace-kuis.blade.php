...
<div class="min-h-screen text-gray-900 dark:text-white flex flex-col font-sans">
    ...
    <header class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#1A1A2E]">
        <div class="flex items-center gap-3">
            <a href="{{ route('instruktur.evaluasi.kuis') }}" class="text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition">
                Kuis
            </a>
        ...
        <div class="flex items-center gap-4">
            ...
            <a href="{{ route('instruktur.evaluasi.kuis') }}" class="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-gray-800 dark:hover:bg-gray-700 text-gray-700 dark:text-white text-sm font-medium rounded-lg transition border border-gray-200 dark:border-gray-700">
                    ...
                    <a href="{{ route('instruktur.evaluasi.kuis.workspace', ['kuis' => $kuis->id, 'pendaftaran_id' => $pendaftaran->id]) }}" 
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            @if($sk && $sk->diselesaikan_pada)
                                <p class="text-[11px] text-gray-500 truncate mt-0.5">Selesai: {{ $sk->diselesaikan_pada->format('d M Y, H:i') }}</p>
                            @else
        ...
        <main class="flex-1 overflow-y-auto p-8 bg-gray-50 dark:bg-[#0F0F1A]">
            @if($selectedPendaftaran)
                <div class="max-w-4xl mx-auto space-y-6">
                    ...
                    <div class="bg-white dark:bg-[#1A1A2E] rounded-xl border border-gray-200 dark:border-gray-800 p-6 flex items-start justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                @if($sesiKuis && $sesiKuis->diselesaikan_pada)
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Diselesaikan pada: {{ $sesiKuis->diselesaikan_pada->format('d F Y, H:i T') }}</p>
                                @else
                        ...
                        @if($sesiKuis)
                            @php
                                $isLate = $kuis->batas_waktu && $sesiKuis->diselesaikan_pada && $sesiKuis->diselesaikan_pada->gt($kuis->batas_waktu);
                            @endphp
                            @if($isLate)
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            @if($sesiKuis)
                                <div class="bg-white dark:bg-[#1A1A2E] rounded-xl border border-gray-200 dark:border-gray-800 p-6">
                                    <h3 class="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-6">Jawaban Peserta</h3>
                                            ...
                                            <div class="border-b border-gray-200 dark:border-gray-800 pb-6 last:border-b-0 last:pb-0">
                                                <div class="mb-3 flex items-start justify-between gap-2">
                                                    ...
                                                    @if ($pertanyaan->tipe_pertanyaan === 'pilihan_ganda')
                                                        <span class="shrink-0 text-xs font-medium px-2 py-0.5 rounded-full {{ $isBenar ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20' }}">
                                                            {{ $isBenar ? '✓ Benar' : '✗ Salah' }}
                                                        </span>
                                                    @else
                                                        <span class="shrink-0 text-xs px-2 py-0.5 rounded-full bg-orange-500/10 text-orange-400 border border-orange-500/20">Esai</span>
                                                    @endif
                                                </div>
                                                ...
                                                @if ($pertanyaan->tipe_pertanyaan === 'pilihan_ganda')
                                                    <div class="space-y-2 ml-4">
                                                        @foreach ($pertanyaan->pilihanJawaban as $pi => $pilihan)
                                                            <div class="flex items-center gap-3 cursor-default p-3 rounded-lg border text-sm
                                                                ...
                                                                <span class="text-gray-700 dark:text-gray-300">
                                                                    <strong class="text-[#6C63FF]">{{ chr(65 + $pi) }}</strong>.
                                                                    {{ $pilihan->teks_pilihan }}
                                                                </span>
                                                                @if ($jawaban && $jawaban->id_pilihan_jawaban == $pilihan->id)
                                                                    <span class="text-xs text-gray-500 ml-auto">(jawaban peserta)</span>
                                                                @endif
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                @else
                                                    <div class="ml-4 p-3 rounded-lg bg-gray-50 dark:bg-[#0B0B14] border border-gray-200 dark:border-gray-800 text-sm text-gray-700 dark:text-gray-300">
                                                        {{ $jawaban?->teks_jawaban ?? '(tidak menjawab)' }}
                                                    </div>
                                                    @php $kunci = $pertanyaan->kunciJawabanEsai->first(); @endphp
                                                    @if ($kunci)
                                                        <div class="ml-4 mt-2 p-3 rounded-lg bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300">
                                                            <span class="font-semibold text-blue-400">Kunci:</span> {{ $kunci->teks_kunci }}
                                                        </div>
                                                    @endif
                                                @endif
                                            </div>
                        ...
                        <div class="lg:col-span-1">
                            @if($sesiKuis)
                                <div class="bg-white dark:bg-[#1A1A2E] rounded-xl border border-gray-200 dark:border-gray-800 p-6 sticky top-8">
                                    ...
                                    <form action="{{ route('instruktur.evaluasi.kuis.workspace.nilai', ['kuis' => $kuis->id, 'pendaftaran' => $selectedPendaftaran->id]) }}" method="POST">
                                        @csrf
                                        ...
                                        <div class="mb-6">
                                            ...
                                            <div class="flex items-center gap-3">
                                                <input type="number" name="nilai" required min="0" max="100" step="0.01"
                                                    value="{{ $sesiKuis->nilaiKuis ? (float) $sesiKuis->nilaiKuis->nilai_mentah : '' }}"
                            ...
                            @else
                                <div class="bg-white dark:bg-[#1A1A2E] rounded-xl border border-gray-200 dark:border-gray-800 p-6 flex flex-col items-center justify-center text-center h-48">
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Peserta belum mengerjakan kuis ini.</p>
                                </div>
                            @endif
                        </div>
                    </div>
            ...
            @else
                <div class="h-full flex flex-col items-center justify-center text-center">
                    ...
                    <p class="text-gray-500 dark:text-gray-400 max-w-sm">Pilih peserta dari daftar di sebelah kiri untuk melihat detail kuis dan memberikan nilai.</p>
                </div>
            @endif
        </main>
    </div>
