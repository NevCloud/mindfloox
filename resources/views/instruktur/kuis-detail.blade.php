                        ...
                        <div class="card translate-none rounded-lg p-6 space-y-4">
                            ...
                            <div class="border-b border-gray-200 dark:border-gray-700">
                                ...
                                <div class="flex flex-wrap gap-3 mb-3">
                                    ...
                                    <span
                                        class="inline-block px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 text-xs font-semibold rounded-full">
                                    ...
                                    <div class="border-b border-gray-200 dark:border-gray-700 pb-4 last:border-b-0">
                                        <div class="mb-3 flex items-start justify-between gap-2">
                                            ...
                                            @if ($pertanyaan->tipe_pertanyaan === 'pilihan_ganda')
                                                <span
                                                    class="shrink-0 text-xs font-medium px-2 py-0.5 rounded-full {{ $isBenar ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' }}">
                                                    {{ $isBenar ? '✓ Benar' : '✗ Salah' }}
                                                </span>
                                            @else
                                                <span
                                                    class="shrink-0 text-xs px-2 py-0.5 rounded-full bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300">Esai</span>
                                            @endif
                                        </div>
                                        ...
                                        @if ($pertanyaan->tipe_pertanyaan === 'pilihan_ganda')
                                            <div class="space-y-2 ml-4">
                                                @foreach ($pertanyaan->pilihanJawaban as $pi => $pilihan)
                                                    <label
                                                        class="flex items-center gap-3 cursor-default p-3 rounded-lg border
                                                        ...
                                                        @if ($jawaban && $jawaban->id_pilihan_jawaban == $pilihan->id)
                                                            <span class="text-xs text-gray-400 ml-auto">(jawaban
                                                                peserta)</span>
                                                        @endif
                                                    </label>
                                                @endforeach
                                        ...
                                        @else
                                            <div
                                                class="ml-4 p-3 rounded-lg bg-gray-100 dark:bg-[#1A1A2E] text-sm text-gray-700 dark:text-gray-300">
                                                {{ $jawaban?->teks_jawaban ?? '(tidak menjawab)' }}
                                            </div>
                                            @php $kunci = $pertanyaan->kunciJawabanEsai->first(); @endphp
                                            @if ($kunci)
                                                <div
                                                    class="ml-4 mt-2 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/10 text-xs text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                                ...
                                @if ($sesiKuis->nilaiKuis)
                                    <div
                                        ...
                                        <p class="text-2xl font-bold text-primary">
                                            {{ number_format($sesiKuis->nilaiKuis->nilai_mentah, 1) }}</p>
                                    </div>
                                @endif

                                <div class="flex items-end gap-3">
                                    <form method="POST"
                                        action="{{ route('instruktur.evaluasi.kuis.nilai', $sesiKuis->id) }}"
                                        class="flex items-end gap-3 flex-1">
                                        @csrf
                                        <div class="flex-1">
                                            <label
                                                class="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide mb-1 block">Nilai
                                                (0–100)</label>
                                    ...
                                    <button type="button"
                                        class="flex items-center gap-2 border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-300 font-semibold py-2.5 px-4 rounded-lg transition duration-200 hover:bg-gray-50 dark:hover:bg-[#1A1A2E]">
