...
@php
    ...
    $avatarUrl = $user->foto_profil
        ...
        : 'https://ui-avatars.com/api/?name=' . urlencode($user->nama) . '&background=6C63FF&color=fff&size=256&font-size=0.4';
        ...
        <div class="flex flex-1 min-w-0 overflow-hidden">
            ...
            <main class="flex-1 min-w-0 flex flex-col overflow-hidden">
                ...
                <div class="flex-1 overflow-y-auto p-5 space-y-5">
                    ...
                    @if(session('success'))
                        <div class="bg-green-500/10 border border-green-500/20 text-green-500 text-sm rounded-xl px-4 py-3 flex items-center gap-2">
                            ...
                            <div class="lg:col-span-3">
                                ...
                                <div class="card-fix p-5 text-left">
                                    ...
                                    <div class="space-y-3 text-left">
                                        ...
                                        <div class="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-300 border-b border-black/5 dark:border-white/5 pb-3">
                                    ...
                                    <!-- Social -->
                                    <div class="mt-5">
                                        <h4 class="text-sm font-semibold dark:text-white mb-3 text-left">Social Media</h4>

                                        <div class="space-y-2">

                                            <!-- LinkedIn -->
                                            <a href="{{ $user->linkedin ?: '#' }}" target="{{ $user->linkedin ? '_blank' : '' }}"
                                            ...
                                            <!-- Instagram -->
                                            <a href="{{ $user->instagram ?: '#' }}" target="{{ $user->instagram ? '_blank' : '' }}"
                                                class="flex items-center justify-between px-3 py-2 rounded-xl bg-gray-100 dark:bg-[#0F0F1A] hover:bg-primary hover:text-white transition text-sm text-gray-600 dark:text-gray-300 group">
                                                <div class="flex items-center gap-3">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <!-- Facebook -->
                                            <a href="{{ $user->facebook ?: '#' }}" target="{{ $user->facebook ? '_blank' : '' }}"
                                                class="flex items-center justify-between px-3 py-2 rounded-xl bg-gray-100 dark:bg-[#0F0F1A] hover:bg-primary hover:text-white transition text-sm text-gray-600 dark:text-gray-300 group">
                                                <div class="flex items-center gap-3">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
    ...
    <div x-show="showEditModal" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0"
        ...
        <div @click.outside="if (!$event.target.closest('.flatpickr-calendar')) showEditModal = false" x-show="showEditModal"
            ...
            <div class="flex items-center justify-between mb-6">
                ...
                <button @click="showEditModal = false" class="text-gray-400 hover:text-gray-600 dark:hover:text-white transition">
            ...
            {{-- Form edit profil --}}
            <form method="POST" action="{{ route($updateRoute) }}" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Nama Lengkap <span class="text-[10px] text-gray-500">(tidak bisa diubah)</span></label>
                    ...
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Username <span class="text-red-400">*</span></label>
                        <div class="relative">
                            <input type="text" name="username" x-model="form.username" @input.debounce.500ms="checkUsername()" required
                            ...
                            <div class="absolute right-3 top-1/2 -translate-y-1/2">
                                <template x-if="usernameChecking"><svg class="w-4 h-4 animate-spin text-primary" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path></svg></template>
                                <template x-if="usernameStatus === 'taken'"><svg class="w-4 h-4 text-red-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg></template>
                                <template x-if="usernameStatus === 'available'"><svg class="w-4 h-4 text-green-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg></template>
                            </div>
                        </div>
                        <p x-show="usernameMessage" class="text-[11px] mt-1" :class="usernameStatus === 'taken' ? 'text-red-400' : 'text-green-400'" x-text="usernameMessage"></p>
                    ...
                    {{-- Role (readonly) --}}
                    <div>
                        <label class="text-xs text-gray-400 mb-1 block">Role <span class="text-[10px] text-gray-500">(tidak bisa diubah)</span></label>
                        <input type="text" value="{{ $roleLabel }}" disabled
                            class="w-full input text-sm text-gray-400 bg-gray-200 dark:bg-white/[0.03] border border-gray-200 dark:border-white/5 rounded-xl focus:outline-none disabled:cursor-not-allowed">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nomor_telepon" x-model="form.nomor_telepon" placeholder="Contoh: +62 812-3456-7890"
                            class="w-full input text-sm dark:text-white bg-gray-50 dark:bg-[#0F0F1A] border border-gray-200 dark:border-white/5 rounded-xl focus:outline-none focus:border-primary">
                    </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="linkedin" x-model="form.linkedin" placeholder="https://linkedin.com/in/..."
                                class="w-full input text-sm dark:text-white bg-gray-50 dark:bg-[#0F0F1A] border border-gray-200 dark:border-white/5 rounded-xl focus:outline-none focus:border-primary">
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" name="facebook" x-model="form.facebook" placeholder="https://facebook.com/..."
                                class="w-full input text-sm dark:text-white bg-gray-50 dark:bg-[#0F0F1A] border border-gray-200 dark:border-white/5 rounded-xl focus:outline-none focus:border-primary">
                        </div>
                    </div>
                </div>
                ...
                {{-- Ganti Kata Sandi --}}
                <div>
                    <h4 class="text-sm font-semibold dark:text-white mb-3">Ganti Kata Sandi <span class="text-xs font-normal text-gray-400">(default: sama dengan username)</span></h4>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                    ...
                    <p class="text-[10px] text-gray-400 mt-1">Kosongkan jika tidak ingin mengubah. Default password = username Anda.</p>
                </div>
