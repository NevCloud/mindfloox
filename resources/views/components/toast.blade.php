@if (session('success') || session('error'))
    <div x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-300 transform"
        ...
        class="fixed bottom-5 right-5 z-50 max-w-sm w-full bg-white dark:bg-[#1A1A2E] shadow-2xl rounded-xl border-l-4 {{ session('success') ? 'border-green-500' : 'border-red-500' }} overflow-hidden flex items-start p-4 gap-4"
