@php
    ...
    if (request()->is('super-admin/*')) {
        $placeholder = 'Cari admin, course, instruktur, dll...';
    } elseif (request()->is('admin/*')) {
        $placeholder = 'Cari verifikasi peserta, course, instruktur, dll...';
    } elseif (request()->is('instruktur/*')) {
        $placeholder = 'Cari course saya, tugas, nilai...';
    } elseif (request()->is('peserta/*')) {
        $placeholder = 'Cari course, tugas, materi...';
    }
@endphp

<div class="flex items-center gap-3 px-4 py-3 bg-gray-100 dark:bg-[#1A1A2E] border-b border-black/5 dark:border-white/5 flex-shrink-0 transition-all duration-300">
    <!-- Mobile left toggle -->
    ...
    <!-- Search -->
    <div class="flex-1 flex items-center gap-2 bg-white dark:bg-[#0F0F1A] border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 transition-all duration-300">
    ...
    <!-- Notification -->
    <button class="relative w-9 h-9 flex items-center justify-center rounded-lg bg-white dark:bg-[#0F0F1A] border border-gray-200 dark:border-gray-700 text-gray-500 hover:text-primary transition">
    ...
    <!-- Mobile right toggle -->
    @if(!request()->is('super-admin/*', 'admin/*'))
